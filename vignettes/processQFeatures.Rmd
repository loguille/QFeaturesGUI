...
QFeatures object can be normalised on this step. The user can choose between different type of normalisation method. 
...
Once the normalisation method is selected you can click on `Apply normalisation` (`A`), this will display the post normalisation density plot (`B`). Once this is done click on `Save the processed sets`. 
...
Join tab will combine the results of previous step into one for downstream analysis or for export, just need a new name for the new dataset. Then click on `Join and save the processed sets`.